...
if [ -d ".claude/agents" ] || [ -L ".claude/agents" ]; then
  PROJECT_AGENTS=$(find -L .claude/agents -maxdepth 1 -name "*.md" -type f 2>/dev/null | wc -l | tr -d ' ')
fi
if [ -d "$HOME/.claude/agents" ] || [ -L "$HOME/.claude/agents" ]; then
  USER_AGENTS=$(find -L "$HOME/.claude/agents" -maxdepth 1 -name "*.md" -type f 2>/dev/null | wc -l | tr -d ' ')
fi
